...
Query.prototype.exec = async function exec(op, callback) {
  // Nếu op là function (callback) thì shift nó sang callback
  ...
  if (this.op == null) {
    const err = new MongooseError('Query must have `op` before executing');
    if (callback) return callback(err);
    throw err;
  }

  if (this.model == null) {
    const err = new MongooseError('Query must have an associated model before executing');
    if (callback) return callback(err);
    throw err;
  }
  ...
  const thunk = opToThunk.get(this.op);
  if (!thunk) {
    const err = new MongooseError('Query has invalid `op`: "' + this.op + '"');
    if (callback) return callback(err);
    throw err;
  }

  if (this.options && this.options.sort && typeof this.options.sort === 'object' && this.options.sort.hasOwnProperty('')) {
    const err = new Error('Invalid field "" passed to sort()');
    if (callback) return callback(err);
    throw err;
  }
